...
async def import_budget(
    ...
    """Import an existing budget Excel file generated by this system and extract its data."""
    try:
        if not file.filename.lower().endswith(('.xlsx',)):
            raise HTTPException(status_code=400, detail="Only .xlsx Excel files are supported")

        content = await file.read()
        parsed = BudgetService.parse_budget_excel(content)
        return parsed
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error importing budget file: {str(e)}")

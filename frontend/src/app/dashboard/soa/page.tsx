...
export default function SOAPage() {
  ...
  const [designationPrepared, setDesignationPrepared] = useState(user?.role || 'Member');
  ...
  const [isImportingSOA, setIsImportingSOA] = useState(false);
  const [importedSOAFileName, setImportedSOAFileName] = useState<string | null>(null);
  ...
  // Sync Prepared By / Designation from logged-in user on first load if empty/default
  useEffect(() => {
    if (user) {
      setPreparedBy((prev) => prev || user.username);
      setDesignationPrepared((prev) => (prev === 'Member' || !prev ? user.role : prev));
    }
  }, [user]);
  ...
  const handleImportExistingSOA = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    setIsImportingSOA(true);
    setImportedSOAFileName(file.name);
    try {
      const result = await soaService.importExistingSOA(file);
      ...
      if (result.activity_code) setActivityCode(result.activity_code);

      if (result.prepared_by) setPreparedBy(result.prepared_by);
      if (result.designation_prepared) setDesignationPrepared(result.designation_prepared);
      if (result.certified_by) setCertifiedBy(result.certified_by);
      if (result.designation_certified) setDesignationCertified(result.designation_certified);
      ...
      alert('Existing SOA Excel imported successfully. You can now modify and regenerate.');
    } catch (error) {
      console.error('SOA import error:', error);
      alert('Error importing existing SOA file. Please ensure it was generated by this system.');
    } finally {
      setIsImportingSOA(false);
      event.target.value = '';
    }
  };
      ...
      <div className="p-4 lg:p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="rounded-lg shadow-sm border border-gray-200 p-6 mb-6" style={{ backgroundColor: '#6698CC' }}>
            ...
            {/* Import Existing SOA Section */}
            <div className="mt-6">
              <h3 className="text-lg font-semibold text-white mb-3">Modify Existing SOA Excel</h3>
              <p className="text-sm text-white opacity-90 mb-3">
                Choose an existing SOA Excel file (generated from this tool) to load and modify.
              </p>
              <div className="flex items-center space-x-4">
                <label className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-black bg-white bg-opacity-20 hover:bg-opacity-30 cursor-pointer">
                  ...
                  <span>{isImportingSOA ? 'Importing...' : 'Choose SOA File'}</span>
                  <input
                    type="file"
                    accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    onChange={handleImportExistingSOA}
                    className="hidden"
                  />
                </label>
                {importedSOAFileName && (
                  <span className="text-sm text-white opacity-90">
                    Loaded: {importedSOAFileName}
                  </span>
                )}
              </div>
            </div>
